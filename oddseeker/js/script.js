    ...
    function rs_filter_results() {
        ...
        jQuery('.filters-container').find('.dropdown').each(function () {
            ...
            jQuery(this).find('input[type=checkbox]:checked, input[type=radio]:checked').each(function () {
                ...
                var rs_filterMobileTitle = jQuery(this).val();
                ...
                // rs_checkedFilters++;
                rs_valFilters =  '<img class="selected-flag" src="' + $(this).siblings('img').attr('src') + '" alt="">' + '<span class="desctop">'+ rs_filterTitle + '</span>' + '<span class="mobile">'+ rs_filterMobileTitle + '</span>';         
            ...
            // }
            // change filters name for selected filter 
            // if (rs_checkedFilters > 0) {
                jQuery(this).find('button span.selected').html(rs_valFilters);
            // } else {
            //     jQuery(this).find('button span.selected').html('');
            // }
        });
        ...
        if (rs_filter_class != '') {
            $rs_providers_list.find('.result-list').hide();
            $rs_providers_list.find('.result-list' + rs_filter_class).show();
        } else {
            $rs_providers_list.find('.result-list').show();
        }

        // filter based on search words
        if (jQuery("#myInput").length) {
            var value = jQuery("#myInput").val().toLowerCase();
            $rs_providers_list.find('.result-list:visible').each(function () {
        ...
        $rs_providers_list.find('li.alphabet-section').show().each(function () {
            if (!jQuery(this).find('.result-list').is(":visible")) {
                jQuery(this).hide();
            }
        });

        // update search result count - Not needed?
        //jQuery('.search-container').find('.result').html($rs_providers_list.find('.result-list:visible').length + ' Results');
    }
